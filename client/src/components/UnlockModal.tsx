...
export function UnlockModal({ open, onOpenChange, onUnlock }: UnlockModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!gdprConsent) {
      setError("Pro pokračování musíte souhlasit se zpracováním osobních údajů.");
      return;
    }
          ...
          <DialogTitle className="text-2xl font-bold text-center font-[Space_Grotesk]">
            {success ? (
              ...
              <span className="text-gradient-gold">
                Odemkněte všech 77 iBotů
              </span>
            )}
          </DialogTitle>
          <DialogDescription className="text-center text-gray-400">
            {success
              ? "Nyní máte přístup ke kompletnímu katalogu AI osobností."
              : "Zadejte svůj e-mail a získejte přístup ke kompletnímu katalogu AI osobností."}
          </DialogDescription>
        </DialogHeader>
          ...
          <>
            ...
            <div className="space-y-3 my-4">
              <div className="flex items-center gap-3 text-sm">
                ...
                <span>Přístup ke všem 77 AI osobnostem ve 7 kategoriích</span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <Zap className="w-5 h-5 text-amber-400 shrink-0" />
                <span>Okamžitá aktivace — nasazení do 5 minut</span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <Users className="w-5 h-5 text-amber-400 shrink-0" />
                <span>Připojte se k 500+ firmám, které už používají iBoty</span>
              </div>
            </div>

            {/* Social proof */}
            <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-3 text-center text-sm text-amber-200/80">
              <span className="font-semibold text-amber-400">127</span> lidí si odemklo katalog za posledních 24 hodin
            </div>
              ...
              >
                {isSubmitting ? (
                  <span className="flex items-center gap-2">
                    <span className="animate-spin">⏳</span> Odemykám...
                  </span>
                ) : (
                  <span className="flex items-center gap-2">
                    <Lock className="w-4 h-4" />
                    Odemknout katalog ZDARMA
                  </span>
                )}
              </Button>

              <p className="text-xs text-gray-500 text-center">
                Žádný spam. Žádná kreditní karta. Okamžitý přístup.
              </p>
            </form>
          </>
        )}
